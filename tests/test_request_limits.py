...
def test_prompt_length_limit_rejects_long_prompt(tmp_path):
    ...
    response = client.post("/generate", json={"prompt": "x" * 100})
...
def test_context_depth_limit_rejects_deep_context(tmp_path):
    ...
    response = client.post("/generate", json={"prompt": "Return wf.vars.a", "context": context})
...
def test_context_width_limit_rejects_wide_context(tmp_path):
    ...
    response = client.post("/generate", json={"prompt": "Return wf.vars.items[1]", "context": context})
