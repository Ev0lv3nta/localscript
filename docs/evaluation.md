...
Для публичных и regression-наборов evidence сохраняет результат каждого кейса,
validation codes, стадии и время. Для закрытого holdout публичный отчёт содержит
только идентификатор и SHA-256 набора, число кейсов, агрегированные метрики и
категории ошибок — без пути к файлу и покейсных идентификаторов.
